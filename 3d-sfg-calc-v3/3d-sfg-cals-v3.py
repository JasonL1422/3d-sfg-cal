...
def calc_ang(n1, n2, incident_angle):
    return np.arcsin((n1 * np.sin(np.radians(incident_angle))) / n2) * (180 / np.pi)
...
def calc_lc(n, lambda_, angle):
    return (2 * np.pi * n / lambda_) * np.cos(np.radians(angle))
...
#---------------------------------------------input2-----------------------------------------------
mode = 2 #1 for reflection 2 for transmission             ###[SELECT]
dk = dkr if mode == 1 else dkt
prefix = 'R' if mode == 1 else 'T'

PClist1 = ["SSP", "PPS", "SSS"]                                  ###[SELECT]
PClist2 = [f"{prefix}{pc1}" for pc1 in PClist1]

dE = [0] # 0.5 ==50%, 1==100%                             ###[SELECT]
theta_angles = [90]                                       ###[SELECT]
sDphi = [7.5]                                             ###[SELECT]
sDtheta = [7.5]                                           ###[SELECT]
dzlist = [5]                                              ###[SELECT]
ulist = [4]                                               ###[SELECT]

iteration = 1000                                          ###[SELECT]
m = 12                                                    ###[SELECT]
#--------------------------------------------------------------------------------------------------

### for tests
tt1_a = np.zeros((25, iteration, m), dtype=np.complex128) # all m OH dipoles, at each #iteration, at each 25 phi angle.
tt1_b = np.zeros((25, iteration), dtype=np.complex128)  # summed OH dipoles, at each #iteration, at each 25 phi angle.
tt1_c = np.zeros((25, iteration), dtype=np.complex128)  # int.  at each #iteration, at each 25 phi angle.
tt1_d = np.zeros(25,dtype=np.complex128)  # mean int. at each 25 phi angle.
#Ori angles
tt1_e = np.zeros((25, iteration, m, 3), dtype=np.float64) # three angles (3), at all m OH dipoles, at each #iteration, at each 25 phi angle.
#seq
tt1_f = np.zeros((25, iteration, m), dtype=np.float64) # all sequences, at each #iteration, at each 25 phi angle.
###
                    ...
                    for sDtheta_val in sDtheta:
                        for sDphi_val in sDphi:
                            ...
                            for phi_temp in range(0, 361, 15):
                                ...
                                #---------------Loop for w...iteration----------------------------------------------------
                                for w in range(iteration):
                                    seq = np.random.permutation(seqtemp)  # randomize the sequence
                                    
                                    OHtemp1 = np.zeros(m, dtype=np.complex128)
                                    CHtemp1 = np.zeros(m, dtype=np.complex128)
                                    
                                    #---------------Loop for i...m crystals-----------------------------------------------
                                    for i in range(m):
                                        Ori1 = gen_ang(phi_temp, theta_fix, 'random', sDphi_val, sDtheta_val, 90) # 90 is a random number. Any is fine when 'random'
                                        Oritemp1 = Ori1
                                        Oritemp2 = {
                                            'phi': Oritemp1['phi'],
                                        ...
                                        ###
                                        tt1_e[phi_index, w, i] = [phi_calc, theta_calc, psi_calc] #-for test
                                        ###
                                        ...
                                        CHtemp1[i] = CHtemp2
                                        ###
                                    #---------------Loop for i...m crystals ends here-----------------------------------------------

                                    tempOH = np.sum(OHtemp1)
                                    tempCH = np.sum(CHtemp1)
                                    G[w] = (np.conj(tempOH) * tempOH) / (np.cos(np.radians(Tangle_SFG)) ** 2) # G #elements=iteration
                                    J[w] = (np.conj(tempCH) * tempCH) / (np.cos(np.radians(Tangle_SFG)) ** 2)
                                    ###
                                    tt1_a[phi_index, w, :] = OHtemp1 # 12 OH dipoles at 5 iteration at 25 degrees
                                    tt1_b[phi_index, w] = tempOH # summed OH dipoles at 5 iteration at 25 degrees
                                    tt1_c[phi_index, w] = G[w] # 5 calculated OH intensity at each iteration at 25 degrees
                                    ###
                                    tt1_f[phi_index, w] = seq # 12 OH dipoles at 5 iteration at 25 degrees
                                    ###
                                #---------------Loop for w...iteration ends here----------------------------------------------------
